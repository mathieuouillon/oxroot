...
use oxroot_hist::{read_tprofile, TProfile};
...
fn round_trips_real_root_tprofile() {
    ...
    oxroot_hist::write_tprofile_file(&out, &h, oxroot_io_core::Compression::None).expect("write");
...
fn writes_a_zstd_compressed_tprofile() {
    ...
    oxroot_hist::write_tprofile_file(&out, &h, oxroot_io_core::Compression::Zstd(5))

...
use oxroot_hist::{append_histograms_file, read_th1d, read_th2d, Hist, TH1, TH2};
...
fn appends_objects_to_an_existing_file() {
    ...
    a.fill(0.5);
    a.fill(2.5);
    oxroot_hist::write_th1d_file(&out, &a, oxroot_io_core::Compression::None)
...
fn re_adding_a_name_bumps_the_cycle() {
    ...
    v1.fill(0.5);
    oxroot_hist::write_th1d_file(&out, &v1, oxroot_io_core::Compression::None).expect("write v1");
    ...
    v2.fill(1.5);
    v2.fill(1.5);
    append_histograms_file(&out, &[Hist::Th1(&v2)], oxroot_io_core::Compression::None)
        .expect("append v2");
